  ...
  return (
    <div className="bg-emerald-50 rounded-xl shadow-lg shadow-emerald-700 py-8 flex flex-col items-center justify-center text-center space-y-5 tracking-wide font-thin border-l-4 md:border-l-8 border-emerald-800 hover:border-l-0 transition-all duration-75">
      <div className="flex items-center justify-center space-x-4">
        <img src={user.picture.medium} alt="avatar" className="rounded-xl size-20 mb-3 border-l-4 border-emerald-800" />
        <div className="flex flex-col space-y-2 items-start justify-center">
          ...
          <div className="text-gray-500 mb-1">Age: <span className="font-normal text-emerald-800">{user.dob.age}</span></div>
          <div className="text-gray-500 mb-2">Contact: <span className="font-normal text-emerald-800">{user.phone}</span></div>
        </div>
      </div>
      <button
        className="mt-auto px-4 py-2 bg-emerald-700 border-l-4 border-emerald-800 text-white rounded-xl hover:bg-emerald-900"
        onClick={onView}>
        View Details
      </button>
    </div>
